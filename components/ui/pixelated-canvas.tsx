...
// Small helper type so we can attach a cleanup callback to the Image
// without resorting to `any` casts throughout the file.
...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = ({
	...
	const revealProgressRef = React.useRef<number>(0);
	...
	React.useEffect(() => {
		...
		const compute = () => {
			...
			// If responsive, prefer the parent container's size so the canvas
			// fills its container and behaves responsively. Fall back to the
			// provided width/height props or the image natural size.
			...
			if (responsive) {
				try {
					...
					if (parent) {
						...
						// Use parent's inner size (CSS pixels). If height isn't available,
						// default to a square based on width.
						if (rect.width > 0) displayWidth = Math.floor(rect.width);
						if (rect.height > 0) displayHeight = Math.floor(rect.height);
						// If parent has width but zero height, keep a square
						if (displayHeight === 0 && displayWidth > 0)
							displayHeight = displayWidth;
					}
				} catch {
					// ignore and fall back to provided sizes
				}
			}
			...
			// keep the offscreen canvas for quick blit when revealing the original
		...
		img.onload = () => {
			...
			const animate = () => {
				...
				// If reveal is active (pointer held), blit the original offscreen image
				if (revealRef.current && offscreenRef.current) {
					try {
						ctx.clearRect(0, 0, dims.width, dims.height);
						ctx.drawImage(offscreenRef.current, 0, 0, dims.width, dims.height);
						rafRef.current = requestAnimationFrame(animate);
						return;
					} catch {
				...
				// Smoothly update reveal progress toward target (0 or 1)
				const revealTarget = revealRef.current ? 1 : 0;
				const revealSpeed = 0.22; // how fast the reveal fades in/out
				revealProgressRef.current =
					revealProgressRef.current +
					(revealTarget - revealProgressRef.current) * revealSpeed;
				...
				for (const s of samples) {
					...
					// lower threshold so small pointer movements still have effect
					if (influence > 0.0001) {
				...
				// If revealProgress > 0, composite the original image on top with that alpha
				const rp = Math.max(0, Math.min(1, revealProgressRef.current));
				if (rp > 0 && offscreenRef.current) {
					try {
						ctx.globalAlpha = rp;
						ctx.drawImage(offscreenRef.current, 0, 0, dims.width, dims.height);
					} catch {
		...
		if (responsive) {
			...
			return () => {
				...
				const c = (img as ImageWithCleanup)._cleanup;
				if (c) c();
			};
		}
